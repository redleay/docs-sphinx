...
EDR（Extended Dynamic Range）是苹果推出的一套渲染管线技术，以支持在不同的屏幕上同时正确显示 SDR 和 HDR 内容。当显示 HDR 的内容时，EDR 并不会直接将 HDR 区域变得更亮，而是识别到 HDR 内容后提高整体屏幕亮度的同时，降低 SDR 区域的白点值，使得尽可能充分利用显示能力和准确还原整体观感。

根据[三十分钟色彩科学——Apple EDR + Metal](https://mp.weixin.qq.com/s/EgJkGimBs5AF1n3O4KqYog)的描述，
现有的显示设备能力都超过标准SDR亮度，但没有完全达到HDR的亮度要求，通常有两种方式使用显示设备相对于SDR多出来的能力：充分利用显示能力的直接映射方式和尽可能准确还原的准确映射方式，直接映射能充分利用显示能力但色彩还原不准确，准确映射的色彩还原准确但浪费了显示能力。
EDR的目的是解决以下两种方式的矛盾，尽可能兼顾两者的优势。
主要的技术原理是，根据不同的环境设置`reference white`，`reference white`以内的亮度范围对应SDR，`reference white`以外的范围对应HDR，
然后根据`reference white`动态调整code value，最后利用Apple提供的默认Tone Mapping算法或定制算法转换为最终的屏幕显示亮度，从而充分利用显示设备的能力，也兼顾了环境的变化，尽可能准确地还原亮度和色彩。
...
[理解 Gainmap 中的 Headroom](https://jackchou00.com/posts/iso-gainmap-alternate-headroom)介绍了Headroom在显示设备的亮度适配上的作用。
