...
| 工具 | 功能  | 系统 | UI | 链接 |
| ---- | ----  | ---- | ---- | ---- |
| MP4Reader | 解析和显示atom | Windows, Linux | CLI | |
| mp4info | 解析和树形显示atom | Windows, Linux | CLI | |
| mp4dump | 解析和树形显示atom | Windows, Linux | CLI | |
| AtomicParsley | 解析文件的atom, 开源，C++编写，长时间未更新，不支持iso4及以上的mp4 | Windows, Linux, MacOS | CLI | [github](https://github.com/wez/atomicparsley/tree/master) |
| Synalyze it | | | | | 
| Elecard | | | | |

| 工具 | 功能  | 系统 | UI | 链接 |
| ---- | ----  | ---- | ---- | ---- |
| mediainfo | 探测媒体信息 | Windows, Linux, MacOS | GUI, CLI | |
| ffprobe | 探测媒体信息 | Windows, Linux, MacOS | GUI, CLI | |
...
mediainfo --Inform="Video;%colour_primaries%,%transfer_characteristics%,%matrix_coefficients%,%colour_range%" input.mp4     # 色彩描述信息
mediainfo --Inform="Video;%MasteringDisplay_ColorPrimaries%,%MasteringDisplay_Luminance%,%MaxCLL%,%MaxFALL%" input.mp4      # HDR静态元数据
magick identify -verbose input.tif
magick identify -format "%wx%h %[colorspace] %[profile:icc]" input.tif
ffprobe -show_streams -threads 8 input.mp4
...
ffmpeg -i input.mp4 -r 0.1 image_%d.jpg                             # 每隔10秒抽1帧，0.1fps即每10秒才有1帧
ffmpeg -i input.mp4 -vf select="eq(n\,${idx})" image_%d.jpg         # 抽出第${idx}帧, number start from 0
...
ffmpeg -i input.mp4 -lavfi scale=w=1920:h=-2:force_original_aspect_ratio=disable:force_divisible_by=2:flags=lanczos -c:v libx265 output.mp4
```
...
图像填充border
```
magick input.jpg -bordercolor "white" -border 24x24 output.jpg  # 四周填充24x24的白色边框
magick input.jpg -gravity southwest -extent 1000x700 -background white output.jpg # 创建1000x700的白色画面，将input.jpg对齐画面southwest，即左下角，相当于上边和右边填充白色
magick input.jpg -gravity northeast -splice 30x0+0+0 -splice 0x30+0+0 -background white output.jpg  # 画面右边和上边填充30白色像素
```
...
# 方法2
使用concat demuxer，`-f concat`, 参考视频时域拼接
```
...
```
# 第一步：生成调色板（优化颜色）
ffmpeg -r 0.5 -pattern_type glob -i "Chang*.png" -vf "fps=0.5,palettegen" palette.png
# 第二步：使用调色板生成高质量GIF
ffmpeg -r 0.5 -pattern_type glob -i "Chang*.png" -i palette.png -lavfi "fps=0.5[x];[x][1:v]paletteuse" output.gif
```

## 流化

mp4流化为hls-ts，生成一个m3u8和多个ts分片

```
# 不加-hls_list_size 0则只输出5个分片
ffmpeg -re -i input.mp4 -c copy -f hls -hls_list_size 0 -bsf:v h264_mp4toannexb hls.m3u8
```

mp4流化为dash-fmp4，生成一个m3u8和`input_dashinit.mp4`文件

```
MP4Box -dash 6 -frag 12 -rap -ref_m3u8 ref.m3u8 input.mp4
```
...
```
left_video=input1.mp4
left_fps=60
left_text=SRC
right_video=input2.mp4
right_fps=60
right_text=HDRv4
ffmpeg -i left_video -i right_video -filter_complex "[0:v] fps=${left_fps} [left]; [1:v] fps=${right_fps} [right]; [left][right] blend=shortest=1:all_expr='if(lte(X/W, (1-2*(mod(T,1)/1)) * mod(floor(T/1),2) + mod(T, 1)/1), A, B)' [merge]; color=c=ffffff:size=2x1048 [line]; [merge][line] overlay=shortest=1:x=round(W * ((1-2*(mod(t\,1)/1)) * mod(floor(t/1)\,2) + mod(t\,1)/1)),drawtext=text=${left_text}:fontsize=50:fontcolor=0xffffff:x=25:y=25:fontfile=PingFang-SC-Regular.ttf,drawtext=text=${right_text}v4:fontsize=50:fontcolor=0xffffff:x=w-25-50*8.5:y=25:fontfile=PingFang-SC-Regular.ttf" -c:v libx264 -x264-params crf=12 -pix_fmt yuv420p -an -y slide_demo-leftSRC-rigthHDRv4.mp4
