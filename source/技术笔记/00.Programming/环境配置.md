...
pip install spleeter">=2.3.1,<=2.4.0"
```
...
缺点是默认路径为`.venv`目录，移动当前目录后会影响虚拟环境配置，不推荐
...
```
conda install python=3.10 opencv py-opencv imageio openimageio av -c conda-forge
```
...
## PEP 668 externally-managed-environment报错

python 3.11之后，采用了新的软件包安装和管理方式，
本报错会出现在运行处于系统包管理工具或conda环境中的`pip install`时触发，报错的目的主要是为了防止pip破坏系统核心包。

解决思路是采用虚拟环境等工具进行环境隔离，可采用以下方式安装应用软件（命令行工具）和软件库（python项目中需要`import`的软件包）。

### 方法1：使用conda

使用conda管理虚拟环境，需要注意的是，需要确认运行的`python/pip`是否是正确的虚拟环境路径版本
```
which python
which pip
```

如不正确，推荐使用`conda install pip`安装`python/pip`即可。
...
sudo yum-config-manager --add-repo http://download.docker.com/linux/centos/docker-ce.repo           # （二选一）官方
sudo yum-config-manager --add-repo http://mirrors.aliyun.com/docker-ce/linux/centos/docker-ce.repo  # （二选一）阿里云
```
...
# 指定高版本
sudo yum install docker-ce-26.1.4-1.el7
```
...
通过dnf升级
```
dnf -y install gcc-toolset-11 # dnf升级

# 激活
source /opt/rh/gcc-toolset-11/enable
# 开机自动激活
cat /opt/rh/gcc-toolset-11/enable >> ~/.bashrc  
```

### 源码编译安装(v7.3.0)

下载源码并解压，并下载依赖的gmp、mpc、mpfr，解压到gcc-7.3.0的根目录后编译安装
...
sudo ln -sf /usr/local/bin/pip3.8 /etc/alternatives/pip3
...
./autogen.sh
./configure --enable-staticlibs # 静态库
...
报错`module 'cv2' has no attribute 'ximgproc'`，运行以下命令修复
```
...
pip安装 (venus容器需要配置代理, 具体参见https://iwiki.woa.com/p/4007830687)
```
...
查看和下载对应的libtorch预编译安装包（将`cu118`替换成自己的版本）
```
...
- 方法1：修改`/etc/ssh/ssh_config`的Port字段为22
- 方法2：在`~/.ssh/config`中增加以下文本
