...
python3自带，不需要额外安装库就能运行，只支持3.3以后版本。缺点是默认路径为`.venv`目录，移动当前目录后会影响虚拟环境配置，不推荐
...
使用conda管理虚拟环境，需要注意的是，需要确认运行的python/pip是否是正确的虚拟环境路径版本
```
which python
which pip
```

如不正确，推荐使用`conda install pip`安装python/pip即可。
...
sudo yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo          # （二选一）官方
